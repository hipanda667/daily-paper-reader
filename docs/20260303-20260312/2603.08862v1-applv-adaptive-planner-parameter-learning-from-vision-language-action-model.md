...
tags: ["query:vla-ar"]
score: 8.0
evidence: 用于导航中自适应规划器参数学习的VLA模型
tldr: 针对移动机器人在受限环境下的导航难题，本文提出APPLV框架。该方法结合了视觉语言模型（VLM）强大的场景理解能力与经典规划器的安全性，通过VLA模型预测规划器参数而非直接输出动作。通过监督学习和强化学习微调，APPLV在BARN数据集和实地实验中展现了优于现有方法的导航性能和泛化能力。
selection_source: fresh_fetch
motivation: 传统导航方法依赖人工调参且泛化性差，而端到端学习和现有VLA模型在复杂受限环境下的精确控制能力不足。
method: 利用预训练视觉语言模型配合回归头预测经典规划器的配置参数，并采用监督学习与强化学习相结合的策略进行微调。
result: 实验结果表明，APPLV在BARN仿真基准和物理机器人测试中的导航成功率及对新环境的适应性均显著超过现有技术。
conclusion: 通过VLA模型动态调整经典规划器参数，能够有效融合基础模型的语义理解力与传统算法的控制精度，提升导航系统的鲁棒性。
---

## 摘要
移动机器人在高度受限环境中的自主导航仍然具有挑战性。经典的导航方法提供了安全保证，但需要针对特定环境进行参数调优；端到端学习绕过了参数调优，但在受限空间中的精确控制方面表现不佳。为此，最近的机器人学习方法在保留经典系统安全性的同时实现了参数调优的自动化，但在泛化到未知环境方面仍面临挑战。最近，视觉-语言-动作（VLA）模型通过利用基础模型的场景理解能力展现出了潜力，但在导航任务中的精确控制和推理延迟方面仍存在困难。在本文中，我们提出了基于视觉-语言-动作模型的自适应规划器参数学习（APPLV）。与直接输出动作的传统 VLA 模型不同，APPLV 利用带有回归头的预训练视觉-语言模型来预测用于配置经典规划器的规划器参数。我们开发了两种训练策略：基于采集的导航轨迹的监督学习微调，以及进一步优化导航性能的强化学习微调。我们在模拟的基准自主机器人导航（BARN）数据集和物理机器人实验中，针对多种运动规划器对 APPLV 进行了评估。结果表明，APPLV 在导航性能和对未知环境的泛化能力方面均优于现有方法。
